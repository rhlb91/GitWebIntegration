		...
		for (RepositoryModel model : repositories) {
			...
			if (model.hasCommits && model.lastChange.after(minimumDate)) {
				Repository repository = repositoryService
						.getRepositoryManager().getRepository(model.name);

				if (repository != null) {
					List<DailyLogEntry> entries = RefLogUtils.getDailyLogByRef(
							model.name, repository, minimumDate, timezone);
					digests.addAll(entries);

					repository.close();
				} else {
					System.out.println("Repository " + model.name
							+ " is null!!");
				}
			} else {
				System.out
						.println("Either repository "
								+ model.name
								+ " not has commits or there are no new changes after !!"
								+ model.lastChange);
			}
		}
		return digests;
	...
	public List<ActivityModel> populateActivities() {
		...
		if (CollectionUtils.isNotEmpty(activities)) {
			for (DailyLogEntry dailyLogEntry : activities) {
				...
				String color = StringUtils.getColor(StringUtils
						.stripDotGit(change.repository));
				...
				if (shortRefName.startsWith(Constants.R_TICKET)) {
					ticketId = shortRefName = shortRefName
							.substring(Constants.R_TICKET.length());
					shortRefName = MessageFormat
							.format("ticket #{0}", ticketId);
					isTicket = true;
				} else if (shortRefName.startsWith(Constants.R_HEADS)) {
					shortRefName = shortRefName.substring(Constants.R_HEADS
							.length());
				} else if (shortRefName.startsWith(Constants.R_TAGS)) {
					shortRefName = shortRefName.substring(Constants.R_TAGS
							.length());
					isTag = true;
				}
				...
				if (isTag) {
					// tags are special
					PersonIdent ident = change.getCommits().get(0)
							.getAuthorIdent();
				...
				default:
					what = MessageFormat.format(
							change.getCommitCount() > 1 ? "{0} commits to"
									: "1 commit to", change.getCommitCount());

					if (change.getAuthorCount() == 1) {
						by = MessageFormat.format("by {0}", change
								.getAuthorIdent().getName());
					} else {
						by = MessageFormat.format("by {0} authors",
								change.getAuthorCount());
					}
					break;
				}
				...
				if (commits.size() > maxCommitCount) {
					commits = new ArrayList<RepositoryCommit>(commits.subList(
							0, maxCommitCount));
				}
	...
	public List<CommitModel> populateCommits(List<RepositoryCommit> commits,
		...
		for (RepositoryCommit commit : commits) {
			...
			if (commit.getName() != null) {
				commitModel.setCommitHash(commit.getName()
						.substring(0, hashLen));
			}
