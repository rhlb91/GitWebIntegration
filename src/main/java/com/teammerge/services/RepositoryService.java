...
public interface RepositoryService extends IManager {

  /**
   * Returns the path of the repositories folder. This method checks to see if Gitblit is running on
  ...
  /**
   * Returns the path of the hooks folder. This method checks to see if Gitblit is running on a
   * cloud service and may return an adjusted path.
   *
  ...
  /**
   * Returns the path of the grapes folder. This method checks to see if Gitblit is running on a
   * cloud service and may return an adjusted path.
   *
  ...
  /**
   * Returns the effective list of permissions for this user, taking into account team memberships,
   * ownerships.
   *
   * @param user
  ...
  /**
   * Returns the list of users and their access permissions for the specified repository including
   * permission source information such as the team or regular expression which sets the permission.
   *
   * @param repository
   ...
   * @since 1.4.0
   */
  boolean setUserAccessPermissions(RepositoryModel repository,
      Collection<RegistrantAccessPermission> permissions);

  /**
   * Returns the list of all users who have an explicit access permission for the specified
   * repository.
   *
  ...
  /**
   * Returns the list of teams and their access permissions for the specified repository including
   * the source of the permission such as the admin flag or a regular expression.
   *
   * @param repository
   * @return a list of RegistrantAccessPermissions
   * @since 1.4.0
   ...
   * @since 1.4.0
   */
  boolean setTeamAccessPermissions(RepositoryModel repository,
      Collection<RegistrantAccessPermission> permissions);

  /**
   * Returns the list of all teams who have an explicit access permission for the specified
   * repository.
   *
  ...
  /**
   * Returns the list of all repositories available to Gitblit. This method does not consider user
   * access permissions.
   *
  ...
  /**
   * Returns a repository model if the repository exists and the user may access the repository.
   *
   * @param user
   * @param repositoryName
  ...
  /**
   * Returns the repository model for the specified repository. This method does not consider user
   * access permissions.
   *
   * @param repositoryName
   * @return repository model or null
  ...
  /**
   * Determines if the specified user has a fork of the specified origin repository.
   *
   * @param username
   * @param origin
  ...
  /**
   * Returns the fork network for a repository by traversing up the fork graph to discover the root
   * and then down through all children of the root node.
   *
   * @param repository
   * @return a ForkModel
   * @since 1.4.0
  ...
  /**
   * Updates the last changed fields and optionally calculates the size of the repository. Gitblit
   * caches the repository sizes to reduce the performance penalty of recursive calculation. The
   * cache is updated if the repository has been changed since the last calculation.
   *
   * @param model
  ...
  /**
   * Returns the metrics for the default branch of the specified repository. This method builds a
   * metrics cache. The cache is updated if the repository is updated. A new copy of the metrics
   * list is returned on each call so that modifications to the list are non-destructive.
   *
   * @param model
   * @param repository
  ...
  /**
   * Creates/updates the repository model keyed by reopsitoryName. Saves all repository settings in
   * .git/config. This method allows for renaming repositories and will update user access
   * permissions accordingly.
   *
   * All repositories created by this method are bare and automatically have .git appended to their
   * names, which is the standard convention for bare repositories.
   *
   * @param repositoryName
   * @param repository
   * @param isCreate
  ...
  /**
   * Deletes the repository from the file system and removes the repository permission from all
   * repository users.
   *
   * @param model
  ...
  /**
   * Returns the list of all Groovy push hook scripts. Script files must have .groovy extension
   *
  ...
  /**
   * Returns the list of pre-receive scripts the repository inherited from the global settings and
   * team affiliations.
   *
  ...
  /**
   * Returns the list of all available Groovy pre-receive push hook scripts that are not already
   * inherited by the repository. Script files must have .groovy extension
   *
  ...
  /**
   * Returns the list of post-receive scripts the repository inherited from the global settings and
   * team affiliations.
   *
   * @param repository if null only the globally specified scripts are returned
   * @return a list of scripts
  ...
  /**
   * Returns the list of unused Groovy post-receive push hook scripts that are not already inherited
   * by the repository. Script files must have .groovy extension
   *
   * @param repository optional parameter
   * @return list of available hook scripts
   * @since 1.4.0
  ...
  /**
   * Ensures that all cached repositories are completely closed and their resources are properly
   * released.
   * 
   * @since 1.4.0
   */
  ...
  /**
   * Ensures that a cached repository is completely closed and it's resources are properly released.
   * 
   * @since 1.4.0
   */
