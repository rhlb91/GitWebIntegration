...
public class MannualDataInsertionCronJob extends AbstractCustomJob {
	...
	public synchronized List<RefModel> runJobSavingForBranchDetails() {
		...
		for (CustomRefModel customRefModel : branchModels) {
			...
			List<RepositoryCommit> commits = CommitCache.instance().getCommits(customRefModel.getRepositoryName(),
			...
			try {
				...
				LOG.error("Cannot create new branch model for repo " + customRefModel.getRepositoryName() + ", branch "
						+ customRefModel.getRefModel().getName() + " from cronjob " + getClass().getSimpleName(), e);
			}
		}
			...
			if (CollectionUtils.isNotEmpty(commits)) {
				for (RepositoryCommit commit : commits) {
					try {
						...
						LOG.error("Cannot create new commit model for repo " + customRefModel.getRepositoryName()
	 ...
	 * </p>
	 * 
	 * <pThis method returns the List of type object, thus the caller method
	 * should type cast the respective object in particular classes before use.
	 * </p>
	 * 
			...
			if (CollectionUtils.isNotEmpty(commits)) {
				for (RepositoryCommit commit : commits) {
					try {
						...
						LOG.error("Cannot create new commit model from cronjob!!" + getClass().getSimpleName(), e);
					}
				}
			}
		}
	...
	public synchronized void fetchAndSaveBranchAndCommitDetailsOnline(String branchId) {
