...
import com.teammerge.model.FilestoreModel;
import com.teammerge.model.GitNote;
import com.teammerge.model.PathModel;
import com.teammerge.model.PathModel.PathChangeModel;
import com.teammerge.model.RefModel;
import com.teammerge.model.SubmoduleModel;
import com.teammerge.model.TicketModel.TicketAction;
import com.teammerge.model.TicketModel.TicketLink;
...
public class JGitUtils {
   ...
   *
   * @param t
   * @param repository if repository is not null it MUST be the {0} parameter in the pattern.
   * @param pattern
   * @param objects
   */
  ...
  /**
   * Clone or Fetch a repository. If the local repository does not exist, clone is called. If the
   * repository does exist, fetch is called. By default the clone/fetch retrieves the remote heads,
   * tags, and notes.
   *
  ...
  /**
   * Fetch updates from the remote repository. If refSpecs is unspecifed, remote heads, tags, and
   * notes are retrieved.
   *
   * @param credentialsProvider
   * @param repository
    ...
    try {
      ...
      if (sharedRepository.isShared()) {
        ...
        if (!JnaUtils.isWindows()) {
          ...
          Iterator<File> iter =
              org.apache.commons.io.FileUtils.iterateFiles(repo.getDirectory(),
  ...
  private enum GitConfigSharedRepositoryValue {
    UMASK("0", 0), FALSE("0", 0), OFF("0", 0), NO("0", 0), GROUP("1", 0660), TRUE("1", 0660), ON(
        "1", 0660), YES("1", 0660), ALL("2", 0664), WORLD("2", 0664), EVERYBODY("2", 0664), Oxxx(
        null, -1);
   ...
   * @return Upon successful completion, a value of 0 is returned. Otherwise, a value of -1 is
   *         returned.
   */
   ...
   *
   * @param repositoriesFolder
   * @param onlyBare if true, only bare repositories repositories are listed. If false all
   ...
   *
   * @param basePath basePath is stripped from the repository name as repositories are relative to
   *        this path
   * @param searchFolder
   * @param onlyBare if true only bare repositories will be listed. if false all repositories are
   *        included.
   * @param searchSubfolders recurse into subfolders to find grouped repositories
      ...
      if (file.isDirectory()) {
        ...
        for (Pattern pattern : patterns) {
          ...
          if (pattern.matcher(path).matches()) {
            LOGGER.debug(MessageFormat.format("excluding {0} because of rule {1}", path,
                pattern.pattern()));
            exclude = true;
            break;
          }
  ...
  /**
   * Returns the first commit on a branch. If the repository does not exist or is empty, null is
   * returned.
   *
   * @param repository
  ...
  /**
   * Returns the date of the first commit on a branch. If the repository does not exist, Date(0) is
   * returned. If the repository does exist bit is empty, the last modified date of the repository
   * folder is returned.
   *
   * @param repository
   * @param branch if unspecified, HEAD is assumed.
  ...
  /**
   * Determine if a repository has any commits. This is determined by checking the for loose and
   * packed objects.
   *
   * @param repository
  ...
  /**
   * Returns the date and author of the most recent commit on a branch. If the repository does not
   * exist Date(0) is returned. If it does exist but is empty, the last modified date of the
   * repository folder is returned.
   *
   * @param repository
  ...
  /**
   * Returns the specified commit from the repository. If the repository does not exist or is empty,
   * null is returned.
   *
   * @param repository
  ...
  public static List<PathModel> getFilesInPath2(Repository repository, String path, RevCommit commit) {
    ...
    try {
      ...
      while (tw.next()) {
        String child =
            isPathEmpty ? tw.getPathString() : tw.getPathString().replaceFirst(
  ...
  /**
   * Returns the list of files changed in a specified commit. If the repository does not exist or is
  ...
  public static List<PathChangeModel> getFilesInRange(Repository repository, String startCommit,
    ...
    try {
      ...
      error(t, repository, "{0} failed to determine files in range {1}..{2}!", startCommit,
          endCommit);
    }
    return list;
  }
  ...
  /**
   * Returns the list of files in the repository on the default branch that match one of the
   * specified extensions. This is a CASE-SENSITIVE search. If the repository does not exist or is
   * empty, an empty list is returned.
   *
   * @param repository
   * @param extensions
  ...
  /**
   * Returns the list of files in the repository in the specified commit that match one of the
   * specified extensions. This is a CASE-SENSITIVE search. If the repository does not exist or is
   * empty, an empty list is returned.
   *
   * @param repository
   ...
   */
  private static PathModel getPathModel(Repository repo, String path, String filter,
    ...
    return new PathModel(pathString, tw.getPathString(), filestoreItem, size, tw.getFileMode(0)
        .getBits(), tw.getObjectId(0).getName(), commit.getName());
  ...
  /**
   * Returns a list of commits starting from the specified objectId using an offset and maxCount for
   * paging. This is similar to LIMIT n OFFSET p in SQL. If the repository does not exist or is
   * empty, an empty list is returned.
   *
   * @param repository
   * @param objectId if unspecified, HEAD is assumed.
  ...
  public static List<RevCommit> getRevLog(Repository repository, String objectId, String path,
    ...
    try {
      ...
      if (!StringUtils.isEmpty(path)) {
        TreeFilter filter =
            AndTreeFilter.create(PathFilterGroup.createFromStrings(Collections.singleton(path)),
                TreeFilter.ANY_DIFF);
  ...
  /**
   * Returns a list of commits for the repository within the range specified by startRangeId and
   * endRangeId. If the repository does not exist or is empty, an empty list is returned.
   *
   * @param repository
  ...
  /**
   * Search the commit history for a case-insensitive match to the value. Search results require a
   * specified SearchType of AUTHOR, COMMITTER, or COMMIT. Results may be paginated using offset and
   * maxCount. If the repository does not exist or is empty, an empty list is returned.
   *
   * @param repository
   * @param objectId if unspecified, HEAD is assumed.
   * @param value
  ...
  public static List<RevCommit> searchRevlogs(Repository repository, String objectId, String value,
      final com.teammerge.Constants.SearchType type, int offset, int maxCount) {
    List<RevCommit> list = new ArrayList<RevCommit>();
      ...
      rw.setRevFilter(new RevFilter() {
        ...
        public boolean include(RevWalk walker, RevCommit commit) throws StopWalkException,
          ...
          switch (type) {
            case AUTHOR:
              include =
                  (commit.getAuthorIdent().getName().toLowerCase().indexOf(lcValue) > -1)
                      || (commit.getAuthorIdent().getEmailAddress().toLowerCase().indexOf(lcValue) > -1);
              break;
            case COMMITTER:
              include =
                  (commit.getCommitterIdent().getName().toLowerCase().indexOf(lcValue) > -1)
  ...
  /**
   * Returns the default branch to use for a repository. Normally returns whatever branch HEAD
   * points to, but if HEAD points to nothing it returns the most recently updated branch.
   *
   * @param repository
  ...
  /**
   * Returns the target of the symbolic HEAD reference for a repository. Normally returns a branch
   * reference name, but when HEAD is detached, the commit is matched against the known tags. The
   * most recent matching tag ref name will be returned if it references the HEAD commit. If no
   * match is found, the SHA1 is returned.
   *
   * @param repository
  ...
  /**
   * Sets the symbolic ref HEAD to the specified target ref. The HEAD will be detached if the target
   * ref is not a branch.
   *
   * @param repository
   * @param targetRef
    ...
    try {
      ...
      switch (result) {
        ...
        case FAST_FORWARD:
          return true;
        default:
          LOGGER.error(MessageFormat.format("{0} {1} update to {2} returned result {3}", repository
              .getDirectory().getAbsolutePath(), branchName, commitId, result));
      }
    } catch (Throwable t) {
  ...
  /**
   * Returns the list of tags in the repository. If repository does not exist or is empty, an empty
   * list is returned.
   *
   * @param repository
   * @param fullName if true, /refs/tags/yadayadayada is returned. If false, yadayadayada is
   *        returned.
  ...
  /**
   * Returns the list of local branches in the repository. If repository does not exist or is empty,
   * an empty list is returned.
   *
   * @param repository
   * @param fullName if true, /refs/heads/yadayadayada is returned. If false, yadayadayada is
   *        returned.
  ...
  /**
   * Returns the list of remote branches in the repository. If repository does not exist or is
   * empty, an empty list is returned.
   *
   * @param repository
   * @param fullName if true, /refs/remotes/yadayadayada is returned. If false, yadayadayada is
   *        returned.
   ...
   *
   * @param repository
   * @param fullName if true, /refs/notes/yadayadayada is returned. If false, yadayadayada is
   *        returned.
  ...
  /**
   * Returns the list of refs in the specified base ref. If repository does not exist or is empty,
   * an empty list is returned.
   *
   * @param repository
   * @param fullName if true, /refs/yadayadayada is returned. If false, yadayadayada is returned.
   * @return list of refs
   */
  ...
  /**
   * Returns a RefModel for the gh-pages branch in the repository. If the branch can not be found,
   * null is returned.
   *
   * @param repository
  ...
  /**
   * Returns a RefModel for a specific branch name in the repository. If the branch can not be
  ...
  /**
   * Returns the submodule definition for the specified path at the specified commit. If no module
   * is defined for the path, null is returned.
   *
   * @param repository
   * @param commit
   * @param path
  ...
  /**
   * Returns the list of notes entered about the commit from the refs/notes namespace. If the
  ...
  public static List<GitNote> getNotesOnCommit(Repository repository, RevCommit commit) {
    ...
    for (RefModel notesRef : noteBranches) {
      ...
      if (!StringUtils.isEmpty(text)) {
        ...
        RefModel noteRef =
            new RefModel(notesRef.displayName, null, history.get(history.size() - 1));
  ...
  /**
   * this method creates an incremental revision number as a tag according to the amount of already
   * existing tags, which start with a defined prefix.
   *
   * @param repository
   * @param objectId
   * @param tagger
  ...
  public static boolean createIncrementalRevisionTag(Repository repository, String objectId,
      PersonIdent tagger, String prefix, String intPattern, String message) {
   ...
   */
  public static boolean createTag(Repository repository, String objectId, PersonIdent tagger,
  ...
  public static boolean createOrphanBranch(Repository repository, String branchName,
    ...
    try {
      ...
      try {
        ...
        ObjectId blobId =
            odi.insert(Constants.OBJ_BLOB, message.getBytes(Constants.CHARACTER_ENCODING));
  ...
  /**
   * Automatic repair of (some) invalid refspecs. These are the result of a bug in JGit cloning
   * where a double forward-slash was injected. :(
   *
   * @param repository
  ...
  /**
   * Returns true if the commit identified by commitId is an ancestor or the the commit identified
   * by tipId.
   *
   * @param repository
   * @param commitId
  ...
  /**
   * Determines if we can cleanly merge one branch into another. Returns true if we can merge
   * without conflict, otherwise returns false.
   *
   * @param repository
   * @param src
   * @param toBranch
  ...
  public static MergeStatus canMerge(Repository repository, String src, String toBranch,
    ...
    IntegrationStrategy strategy =
        IntegrationStrategyFactory.create(mergeType, repository, src, toBranch);
  ...
  public static MergeResult merge(Repository repository, String src, String toBranch,
      MergeType mergeType, PersonIdent committer, String message) {
  ...
  private static class FastForwardOnly extends IntegrationStrategy {
    ...
    MergeResult _merge(PersonIdent committer, String message) throws IOException {
      ...
      operationMessage =
          MessageFormat.format("fast-forwarding {0} to commit {1}", srcTip.getName(),
              branchTip.getName());
  ...
  private static class MergeIfNecessary extends IntegrationStrategy {
    ...
    MergeStatus _canMerge() throws IOException {
      ...
      RecursiveMerger merger =
          (RecursiveMerger) MergeStrategy.RECURSIVE.newMerger(repository, true);
    ...
    MergeResult _merge(PersonIdent committer, String message) throws IOException {
      if (revWalk.isMergedInto(branchTip, srcTip)) {
        ...
        refLogMessage = "merge " + src + ": Fast-forward";
        operationMessage =
            MessageFormat.format("fast-forwarding {0} to commit {1}", branchTip.getName(),
                srcTip.getName());
      ...
      if (merged) {
        ...
        try {
          ...
          if (StringUtils.isEmpty(message)) {
            message =
                MessageFormat.format("merge {0} into {1}", srcTip.getName(), branchTip.getName());
          }
          ...
          operationMessage =
              MessageFormat.format("merging commit {0} into {1}", srcTip.getName(),
                  branchTip.getName());
  ...
  private static class IntegrationStrategyFactory {
    static IntegrationStrategy create(MergeType mergeType, Repository repository, String src,
        String toBranch) {
      switch (mergeType) {
  ...
  /**
   * Returns the LFS URL for the given oid Currently assumes that the Gitblit Filestore is used
   *
   * @param baseURL
   ...
   * @throws IOException
   */
  public static List<DirCacheEntry> getTreeEntries(Repository db, String branch,
      Collection<String> ignorePaths) throws IOException {
  ...
  public static boolean commitIndex(Repository db, String branch, DirCache index,
      ObjectId parentId, boolean forceCommit, String author, String authorEmail, String message)
    ...
    try {
      ...
      commit.setEncoding(com.teammerge.Constants.ENCODING);
      commit.setMessage(message);
   ...
   */
  @NotNull
  public static List<TicketLink> identifyTicketsFromCommitMessage(Repository repository,
      IStoredSettings settings, RevCommit commit) {
   ...
   */
  public static List<TicketLink> identifyTicketsBetweenCommits(Repository repository,
      IStoredSettings settings, String baseSha, String tipSha) {
  ...
  public static int countCommits(Repository repository, RevWalk walk, ObjectId baseId,
      ObjectId tipId) {
    int count = 0;
    walk.reset();
    walk.sort(RevSort.TOPO);
