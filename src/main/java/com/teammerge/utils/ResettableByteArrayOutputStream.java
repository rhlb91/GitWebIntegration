...
public class ResettableByteArrayOutputStream extends ByteArrayOutputStream {
  ...
  /**
   * Reset the stream to the given position. If {@code mark} is <= 0, see {@link #reset()}. A no-op
   * if the stream contains less than {@code mark} bytes. Otherwise, resets the current writing
   * position to {@code mark}. Previously allocated buffer space will be reused in subsequent
   * writes.
   *
