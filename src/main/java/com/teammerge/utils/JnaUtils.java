...
public class JnaUtils {
  ...
  public static int getgid() {
    if (isWindows()) {
      throw new UnsupportedOperationException(
          "The method JnaUtils.getgid is not supported under Windows.");
    }
  ...
  public static int getegid() {
    if (isWindows()) {
      throw new UnsupportedOperationException(
          "The method JnaUtils.getegid is not supported under Windows.");
    }
   ...
   *
   * The permission bits are set to the provided mode. This method is only implemented for OSes of
   * the Unix family and makes use of the 'chmod' function of the native C library. See 'man 2
   * chmod' for more information.
   *
   * @param path File/directory to set the permission bits for.
   * @param mode A mode created from or'd permission bit masks S_I*
   * @return Upon successful completion, a value of 0 returned. Otherwise, a value of -1 is
   *         returned.
   */
  ...
  public static int setFilemode(String path, int mode) {
    if (isWindows()) {
      throw new UnsupportedOperationException(
          "The method JnaUtils.getFilemode is not supported under Windows.");
    }
   ...
   * @return Upon successful completion, the file mode bits are returned. Otherwise, a value of -1
   *         is returned.
   */
   ...
   *
   * This method is only implemented for OSes of the Unix family. It returns file status information
   * for a file. Currently this is the file mode, the user id and group id of the owner.
   *
   * @param path File/directory to get the file status from.
   * @return Upon successful completion, a Filestat object containing the file information is
  ...
  public static Filestat getFilestat(String path) {
    if (isWindows()) {
      throw new UnsupportedOperationException(
          "The method JnaUtils.getFilestat is not supported under Windows.");
    }
    ...
    // This could be done with the 'stat' command, too. But that may have a shell specific
    // implementation, so we use 'ls' instead.
    ...
    Pattern p =
        Pattern
            .compile("^(([-bcdlspCDMnP?])([-r][-w][-xSs])([-r][-w][-xSs])([-r][-w][-xTt]))[@+.]? +[0-9]+ +([0-9]+) +([0-9]+) ");
   ...
   * @return The first line of output from the 'ls' command. Null, if an error occurred and no line
   *         could be read.
   */
