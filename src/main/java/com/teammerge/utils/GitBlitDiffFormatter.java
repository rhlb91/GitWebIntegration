...
import com.teammerge.utils.DiffUtils.BinaryDiffHandler;
import com.teammerge.utils.DiffUtils.DiffStat;

/**
 * Generates an html snippet of a diff in Gitblit's style, tracks changed paths, and calculates diff
 * stats.
 *
 * @author James Moger
...
public class GitBlitDiffFormatter extends DiffFormatter {
   ...
   * CSS rules (despite my having optimized them). And second, no human can read a diff with
   ...
   * produced by the super implementation. Unfortunately the super implementation has far too many
   * things private; otherwise we'd just have re-implemented
   * {@link GitBlitDiffFormatter#format(DiffEntry) format(DiffEntry)} completely without ever
  ...
  private static class DiffOutputStream extends ResettableByteArrayOutputStream {
    ...
    public void write(byte[] b, int offset, int length) {
      if (binaryDiffHandler != null
          && RawParseUtils.decode(Arrays.copyOfRange(b, offset, offset + length)).contains(
              BINARY_DIFFERENCE)) {
  ...
  public GitBlitDiffFormatter(String commitId, Repository repository, String path,
    ...
    // If we have a full commitdiff, install maxima to avoid generating a super-long diff listing
    // that
    // will only tax the browser too much.
    maxDiffLinesPerFile =
        path != null ? -1 : getLimit(DIFF_LIMIT_PER_FILE_KEY, 500, DIFF_LIMIT_PER_FILE);
  ...
  private void handleChange() {
    ...
    switch (entry.getChangeType()) {
      ...
      case RENAME:
        message =
            MessageFormat.format(getMsg("gb.diffRenamedFile", "File was renamed from {0}"),
                entry.getOldPath());
        break;
      case COPY:
        message =
            MessageFormat.format(getMsg("gb.diffCopiedFile", "File was copied from {0}"),
                entry.getOldPath());
        break;
      default:
        return;
   ...
   *
   * @param prefix the diff prefix (+/-) indicating whether the line was added or removed.
   * @param line the line to format as HTML
    ...
    if ((prefix == '+' || prefix == '-')) {
      ...
      if (matcher.find()) {
        StringBuilder result =
            new StringBuilder(StringUtils.escapeForHtml(line.substring(0, matcher.start()),
                CONVERT_TABS, tabLength));
