...
import com.teammerge.model.AnnotatedLine;
...
public class DiffUtils {
  ...
  /**
   * Callback interface for binary diffs. All the getDiff methods here take an optional handler; if
   * given and the {@link DiffOutputType} is {@link DiffOutputType#HTML HTML}, it is responsible for
   * displaying a binary diff.
   */
  public interface BinaryDiffHandler {

    /**
     * Renders a binary diff. The result must be valid HTML, it will be inserted into an HTML table
     * cell. May return {@code null} if the default behavior (which is typically just a textual note
     * "Bnary files differ") is desired.
     *
     * @param diffEntry current diff entry
     *
   ...
   * @param comparator
   * @param outputType
   * @param handler to use for rendering binary diffs if {@code outputType} is
   *        {@link DiffOutputType#HTML HTML}. May be {@code null}, resulting in the default
   *        behavior.
   * @param tabLength
   * @return the diff
   */
  ...
  /**
   * Returns the diff for the specified file or folder from the specified commit compared to its
   * primary parent.
   *
   * @param repository
   * @param commit
   ...
   *
   * @param repository
   * @param base if base commit is unspecified, the diffstat is generated against the primary parent
   *        of the specified tip.
   * @param tip
   * @param path if path is specified, the diffstat is generated only for the specified file or
   *        folder. if unspecified, the diffstat is generated for the entire diff between the two
   *        commits.
   * @return patch as a string
   */
   ...
   *
   * @param repository
   * @param baseCommit if base commit is unspecified, the diffstat is generated against the primary
   *        parent of the specified commit.
   * @param commit
   * @param path if path is specified, the diffstat is generated only for the specified file or
   *        folder. if unspecified, the diffstat is generated for the entire diff between the two
   ...
   * @return patch as a string
   */
  public static DiffStat getDiffStat(Repository repository, RevCommit baseCommit, RevCommit commit,
      String path) {
    DiffStat stat = null;
    try {
      RawTextComparator cmp = RawTextComparator.DEFAULT;
  ...
  /**
   * Returns the list of lines in the specified source file annotated with the source commit
   * metadata.
   *
   * @param repository
   * @param blobPath
  ...
  public static List<AnnotatedLine> blame(Repository repository, String blobPath, String objectId) {
    ...
    try {
      ...
      LOGGER.error(
          MessageFormat.format("failed to generate blame for {0} {1}!", blobPath, objectId), t);
    }
    return lines;
  }
